...
{
    // Right Panel userGroupDataSource
    private String DeleteCommandUserGroup = "DELETE FROM [UserGroup] WHERE [userID] = {0}";
    private String InsertCommandUserGroup = "INSERT INTO [UserGroup] ([groupID], [userID]) VALUES ({0},{1})";
    private String SelectCommandUserGroup = "SELECT [Group].[ID], [groupName] FROM [UserGroup] left join [Group] on [Group].[ID] = [UserGroup].[groupID] and [UserGroup].[userID] = {0} where [Active] = 1";
    private String UpdateCommandUserGroup = "";

    // Left Panel groupDataSource
    ...
    private String SelectCommandGroup = "SELECT [Group].[ID], [groupName] FROM [Group] left outer join [UserGroup] on [Group].[ID] = [UserGroup].[groupID] and [UserGroup].[userID] = {0} where [UserGroup].[groupID] is NULL and [Active] = 1";
    ...
    protected void ButtonEdit_Click(object sender, EventArgs e)
    {
        ...
        groupDataSource.SelectCommand = String.Format(SelectCommandGroup, stuff);
        userGroupDataSource.SelectCommand = String.Format(SelectCommandUserGroup, stuff);
    }
    ...
    protected void ButtonDelete_Click(object sender, EventArgs e)
    {
        ...
        var item = (ListViewItem)btn.NamingContainer;
        Label thelabel = (Label)item.FindControl("IDLabel");
        userId = thelabel.Text;

        userGroupDataSource.DeleteCommand = String.Format(DeleteCommandUserGroup, userId);
        userGroupDataSource.Delete();
    ...
    protected void ButtonUpdate_Click(object sender, EventArgs e)
    {
        ...
        {
            userGroupDataSource.InsertCommand = String.Format(InsertCommandUserGroup, ListBox2.Items[i].Value, userId);
            userGroupDataSource.Insert();
        }
    }
}